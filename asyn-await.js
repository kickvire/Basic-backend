...
//Promise way to handle asyn operation
// getMovieFromDatabase(id)//can we attach then to any function ,NO only for those which return promise(asyn operation)
//     .then(movie => getDirectorOfMovie(movie.name))
//     .then(director => getNumberOfHitsForTheDirector(director.directorName))
//     .then(hits => console.log(hits))
//     .catch((ex) => console.log("Ex", ex));

//it is jst a syntactic sugar on promises except use try and catch to catch the error instead of then and catch
async function getNumberOfHits(){
const movie= await getMovieFromDatabase(id);
const director= await getDirectorOfMovie(movie.name);
const hit= await getNumberOfHitsForTheDirector(id);
console.log(hits);
}
